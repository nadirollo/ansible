...
With the following Ansible playbook you can find the VMware ESXi host system(s) and can perform various tasks depending on the list of host system.
...
Since Ansible utilizes the VMware HTTP API using ``uri`` module to perform actions, in this use case it will be connecting directly to the VMware HTTP API from localhost.
...
As you can see, we are using ``uri`` module in first task to login into the vCenter server and storing result in ``login`` variable using register. In second task, using cookies from first task we gathering information about ESXi host system.

Using this information, we are changing ESXi host system's advance configuration.
